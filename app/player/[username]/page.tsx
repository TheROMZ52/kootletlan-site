...
async function nameFrom(params: Props['params']) {
  ...
  let decoded = ''
  try { decoded = decodeURIComponent(username).trim() } catch { /* malformed escape */ }
  return decoded
}
...
export default async function PlayerPage({ params }: Props) {
  ...
  try {
    player = await getPlayerByUsername(name)
  } catch (error) {
    console.error('[player] lookup failed:', error)
    return (
      <>
        <PageHeader title={name} lead="الان نمی‌توانیم اطلاعات این بازیکن را بگیریم. کمی بعد دوباره سر بزن." />
      </>
    )
  }
  if (!player) notFound()
