...
export default async function AdminPage() {
  ...
  return <div className="container">
    ...
    <div className="admin-stat-grid">
      ...
      <div className="admin-stat"><strong>{online[0]?.count ?? 0}</strong><span>آنلاین</span></div></div>
      <div className="admin-stat"><strong>{tickets[0]?.count ?? 0}</strong><span>تیکت باز</span></div></div>
      <div className="admin-stat"><strong>{news[0]?.count ?? 0}</strong><span>خبرها</span></div></div>
    </div>
